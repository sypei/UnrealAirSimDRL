    ...
    def get_event(self):
        try:
            ...
            if self._dev_file in r:
                buf = self._dev_file.read(8)#8 bytes
            else:
                return
        except ValueError:
    ...
    def process_event(self, event):
        if event.type == JS_EVENT_BUTTON:
            try:
                ...
                button._last_value = button._value
        ...
        if event.type == JS_EVENT_AXIS:
            if self.raw_mode:
                ...
                if num == 0:
                    self.axis_l._last_value_x = self.axis_l._value_x
                    self.axis_l._value_x = val 
                if num == 1:
                    self.axis_l._last_value_y = self.axis_l._value_y
                    self.axis_l._value_y = val
                if num == 2:
                    self.trigger_l._last_value = self.trigger_l._value
                    self.trigger_l._value = (val+1)/2
                if num == 3:
                    self.axis_r._last_value_x = self.axis_r._value_x
                    self.axis_r._value_x = val
                if num == 4:
                    self.axis_r._last_value_y = self.axis_r._value_y
                    self.axis_r._value_y = val
                if num == 5:
                    self.trigger_r._last_value = self.trigger_r._value
                    self.trigger_r._value = (val+1)/2
                if num == 6:
                    self.hat._last_value_x = self.hat._value_x
                    self.hat._value_x = int(val)
                if num == 7:
                    self.hat._last_value_y = self.hat._value_y
